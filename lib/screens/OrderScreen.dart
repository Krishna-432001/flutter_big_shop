...
class _OrderScreenState extends State<OrderScreen> {
  ...
  Future<void> fetchData() async {
    ...
    try {
      final response = await http.get(Uri.parse(Constants.BASE_URL + Constants.ORDER_ROUTE));
      ...
      print('Error: $e');
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed to load orders. Please try again later.')));
    } finally {
      setState(() {
        isLoading = false;
      });
  ...
  void navigateToOrderDetailScreen(Order order) {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => OrderDetailScreen(order: order, title: 'order',)),
    );
  }
